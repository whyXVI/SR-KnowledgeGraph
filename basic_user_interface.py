...
model_chat_engine = "gpt-3.5-turbo-0125"

SYSTEM_MESSAGE = (
            "You are a helpful professor and polymath scientist. You want to help a fellow researcher learn more about the world. "
            + "You are clear, concise, and precise in your answers, and you follow instructions carefully.")


def _gen_chat_response(prompt='hi'):
    response = openai.chat.completions.create(model=model_chat_engine,
                                              messages=[
...
def extract_abstraction_groups(knowledgeGraph,
    ...
    if answer is None:
        example_cards_and_concept_lists = chain_card_example_objects(["question", "abstraction_groups"], used_cardIDs,
                                                                     knowledgeGraph)
    else:
        example_cards_and_concept_lists = chain_card_example_objects(["question", "answer", "abstraction_groups"],
                                                                     used_cardIDs, knowledgeGraph)
...
def get_answers_to_questions(knowledgeGraph,
                             ...
                             question_subject_list: List[str] = [],
                             # the subjects to find relevant things in the knowledge graph to display
    ...
    prompt = (
                "Professor Smith has provided the following question and answer pairs to the class to review as flashcards. "
...
def get_suggested_further_questions(knowledgeGraph,
    ...
    if (question_subject_list_in_graph is not None) and num_related_cards_in_graph_to_show > 0:
        ...
        related_questions_text_unwrapped = "".join(
...
class QAEditApp(tk.Toplevel):
    ...
    def save_and_reset(self,
        ...
        if len(self.question_answer_pairs) > 0:
            new_cards_df = load_question_answer_pairs_into_knowledgeGraph(self.question_answer_pairs, self.cards_df,
                                                                          self.knowledgeGraph)
...
class ExplorationDataEditor:
    def __init__(self, master, exploration_data, knowledgeGraph, cards_df):
        ...
        # Query button
        row_now += 1
        self.new_questions_button = tk.Button(self.master, text="Generate new questions",
                                              command=self.generate_new_questions)
        self.new_questions_button.grid(row=row_now, columnspan=2)
        # Query button
        row_now += 1
        self.situation_in_graph_button = tk.Button(self.master, text="Gather related questions from graph",
                                                   command=self.update_related_questions)
        ...
        # Print button
        row_now += 1
        self.answer_button = tk.Button(self.master, text="Print related questions and answers",
                                       command=self.print_related_questions_and_answers)
        self.answer_button.grid(row=row_now, columnspan=2)
        ...
        self.update_button = tk.Button(self.new_question_window, text="Update Questions",
                                       command=self.update_new_questions)
        self.update_button.grid(row=0, column=1, columnspan=1)
        self.update_button_toggle = tk.Button(self.new_question_window, text="Tog.",
                                              command=self.update_new_questions_toggle_all)
    ...
    def update_related_questions(self):
        ...
        related_question_list = list(
            reversed([self.knowledgeGraph.cards[cardID].question for cardID in related_cardIDs]))
    ...
    def redisplay_related_questions(self):
        ...
        question_label = tk.Label(self.related_question_window,
                                  text='----------------- Below here not displayed -----------------',
